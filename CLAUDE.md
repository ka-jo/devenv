...
2. **`devcontainer/`** — a shared base `docker-compose.yml`, referenced directly (via `-f`, never copied) by `devenv container up/down/attach` for every `worktrees/<repo>/<branch>` checkout. Debian-Trixie app container (runs as non-root user `ka-jo`; zsh + fnm + pnpm + Claude Code) plus a Squid egress firewall sidecar on an `internal: true` Docker network; default-deny allowlist seeded per-worktree from `firewall/allowed_domains.txt.default`. The app container is gated on the firewall's health check (`depends_on: service_healthy`); `firewall/start.sh` watches the allowlist and SIGHUPs Squid to reload it live. The `firewall/` dir is re-mounted **read-only** into the app container (overlaying the rw `/workspace` mount) so a sandboxed process can't widen its own egress — the allowlist is edited from the host only; don't remove that `:ro` overlay. There is no `devcontainer.json` and VS Code does not own the container lifecycle — containers are started/stopped via the `devenv container` CLI and VS Code, if used, attaches to an already-running container.
...
- **Per-worktree container state (`<worktree>/.devcontainer/`: `.env`, seeded `firewall/allowed_domains.txt`/`denied_domains.txt`) is generated by `devenv container up`, never committed.** It's excluded via `worktrees/<repo>/.git/info/exclude` (written once per repo — that file lives in the bare repo's common git dir, so it covers every worktree of that repo automatically). Firewall lists are seeded once from `devcontainer/firewall/*.default` and then preserved forever (skip-if-exists) — edit them in the worktree's `.devcontainer/firewall/`, not the `.default` templates, to change one worktree's allowlist.
...
- `devenv container up <repo> <branch>` — start (build if needed) a worktree's dev container stack via the shared base `devcontainer/docker-compose.yml`; seeds per-worktree firewall allowlists and regenerates `.env` (`WORKSPACE_DIR`, `COMPOSE_PROJECT_NAME`) each run.
- `devenv container down <repo> <branch>` — stop that worktree's stack.
